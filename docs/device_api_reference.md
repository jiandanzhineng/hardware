...
- [DZC01 电子秤](#dzc01-电子秤)
- [CUNZHI01 寸止01设备](#cunzhi01-寸止01设备)
- [MQTT通信协议](#mqtt通信协议)
- [错误代码](#错误代码)
...
- **指示灯闪烁**: 收到 `{"method":"action","action":"blink"}` 时，支持 connected LED 的设备会快速闪烁3秒
...
**设备类型**: `CUNZHI01`
**描述**: 集成跳蛋电机、双通道压力监测及高压电脉冲刺激的多功能设备

### 设备属性

| 属性名            | 类型  | 读写权限 | 描述               | 默认值 | 范围      |
| ----------------- | ----- | -------- | ------------------ | ------ | --------- |
| `power`           | int   | 读写     | 电机震动强度       | 0      | 0-255     |
| `voltage`         | int   | 读写     | 电脉冲强度         | 0      | 0-100     |
| `pressure`        | int   | 只读     | 压力通道1数值(欧姆)| 0      | 0-99999   |
| `pressure1`       | int   | 只读     | 压力通道2数值(欧姆)| 0      | 0-99999   |
| `report_delay_ms` | int   | 读写     | 状态上报周期(ms)   | 1000   | 100-10000 |

### 功能说明

- **电机控制**: 通过PWM控制震动强度 (`power`)
- **电脉冲刺激**: 通过升压电路和H桥输出双向电脉冲，强度可调 (`voltage`)
- **压力监测**: 双通道压力传感器实时监测挤压状态
- **电池管理**: 自动监测电池电压

### 硬件接口

- **GPIO 7**: 电机控制 (PWM) / LED指示灯
- **GPIO 6**: 电脉冲升压控制 (PWM)
- **GPIO 4/5**: 电脉冲方向控制
- **GPIO 2/3**: 压力传感器ADC输入
- **GPIO 0/1**: 电池电压检测
