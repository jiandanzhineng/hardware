...
> 本文主要描述硬件接口、属性、协议与固件实现。  
> 若要看产品层的手机/电脑玩法与当前主推方向，请先阅读 [CUNZHI01产品定义](CUNZHI01产品定义.md)。
...
| `safe` | int | 读写 | 安全模式开关，限制电脉冲最大电压 | 1 | 0-1 (1=安全模式限36V, 0=放开至75V) |
...
- **安全模式 (`safe`)**: 限制电脉冲升压输出的最大电压。
  - `safe = 1`（默认，安全模式）：最大电压限制为 **36V**。
  - `safe = 0`：放开限制至 **75V**。
  - 该限制同时作用于直接电压设置 (`boost_control`) 与升压渐变过程，超过上限的目标值会被截断。
  - `safe` 写入后持久化到 NVS（`cunzhi01_store`），设备重启后自动恢复，默认值为 1。
