...
void init_properties(void) {
    ...
    pressure2_property.readable = true;
    pressure2_property.writeable = false;
    strcpy(pressure2_property.name, "pressure2");
    pressure2_property.value_type = PROPERTY_TYPE_FLOAT;
    pressure2_property.value.float_value = 0;
...
static void boost_control(int voltage) {
    ...
    // Frequency control similar to dianji.c (PFM)
    // Map voltage 1-100 to frequency 100-20000 Hz
    uint32_t freq = 100 + (uint32_t)voltage * (20000 - 100) / 100;
    freq = 10000;
    
    // Set frequency
    esp_err_t err = ledc_set_freq(BOOST_MODE, BOOST_TIMER, freq);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "ledc_set_freq failed: %s, re-init timer", esp_err_to_name(err));
        // Fallback: re-configure timer if set_freq fails (e.g. resolution not found)
        ledc_timer_config_t boost_timer = {
            .speed_mode = BOOST_MODE,
            .duty_resolution = BOOST_DUTY_RES,
            .timer_num = BOOST_TIMER,
        ...
        ledc_timer_config(&boost_timer);
    }
    
    // Fixed duty cycle (approx 75%, 192/255)
    uint32_t fixed_duty = 192;
    fixed_duty = (uint32_t)voltage;
    ledc_set_duty(BOOST_MODE, BOOST_CHANNEL, fixed_duty);
    ledc_update_duty(BOOST_MODE, BOOST_CHANNEL);

    ESP_LOGI(TAG, "Boost freq: %lu, duty: %lu", freq, fixed_duty);
}
...
static void pressure_task(void *arg) {
    while (1) {
        ...
        float r11 = 0.5 * p1 + 0.5 * pressure1_property.value.float_value;
        device_update_property_float("pressure1", r11);

        float r22 = 0.5 * p2 + 0.5 * pressure2_property.value.float_value;
        device_update_property_float("pressure2", r22);

        float max_p = (r11 > r22) ? r11 : r22;
        device_update_property_float("pressure", max_p);
...
static void report_task(void *arg) {
    while (1) {
        ...
        cJSON_AddNumberToObject(root, "pressure2", pressure2_property.value.float_value);
